...
# Gumbel TR
q_tr = []
gumbel_tr = [(i+1.)*10 for i in tqdm.tqdm(range(len(q_max)))]
...
vetor_tr_pearson = [(i+1.)*10 for i in tqdm.tqdm(range(len(q_max)))]
...
pearson_plot = pd.DataFrame({'QMax (m3/s)' : pearson_tr, 'TR' : vetor_tr_pearson})
pearson_plot.plot.scatter(x='TR', y='QMax (m3/s)', title='Log-Pearson Tipo III', c='DarkGreen')