...
O rio Grande foi o único que a análise foi feita em uma massa reduzida de dados. Conforme já detalhado, apenas dados de setembro de 2020 até dezembro de 2023. Feita esta ressalva, os resultados mostraram-se bons e os modelos foram estáveis quando da análise dos resíduos.
...
%A análise dos gráficos de resíduos versus valores previstos para os três modelos — CatBoost (CB), Regressão Linear (LR) e RandomForest (RF) — revela importantes diferenças no comportamento dos resíduos em relação às previsões. Esses gráficos são fundamentais para entender como cada modelo lida com erros em diferentes intervalos de previsão e identificar padrões ou tendências que possam indicar deficiências nos modelos.
%
%### 1. **Distribuição Geral dos Resíduos**
%
%- **CatBoost (CB)**: 
%- Os resíduos estão relativamente concentrados em torno de zero para a maioria dos valores previstos. No entanto, à medida que os valores previstos aumentam, os resíduos se tornam mais dispersos, especialmente para previsões acima de 6.000 \(m^3/s\). Isso indica que o CB tende a subestimar ou superestimar mais frequentemente as previsões para valores maiores de vazão.
%- Existem vários outliers positivos e negativos, com valores de resíduos superiores a 2.000 \(m^3/s\) e inferiores a -1.000 \(m^3/s\), principalmente para as previsões mais altas (acima de 6.000 \(m^3/s\)).
%- **Conclusão**: O CB lida bem com vazões mais baixas e moderadas, mas apresenta maior dificuldade com picos extremos de vazão, como indicado pela dispersão e pelos outliers em previsões de alta magnitude.
%
%- **Regressão Linear (LR)**:
%- A LR apresenta uma distribuição de resíduos semelhante ao CB em termos de concentração em torno de zero para valores previstos mais baixos. No entanto, a dispersão dos resíduos é maior, e há uma maior quantidade de outliers, especialmente para previsões de valores mais altos (acima de 7.000 \(m^3/s\)).
...
%
%### 2. **Tendências nos Resíduos**
%
%- **CatBoost (CB)**: 
%- A tendência de dispersão crescente dos resíduos com o aumento dos valores previstos sugere que o CB tem dificuldade em prever com precisão vazões elevadas. Não há um padrão claro de subestimação ou superestimação sistemática, mas os valores de resíduos mais altos indicam que o modelo pode não estar capturando adequadamente as variações extremas nas vazões.
%
%- **Regressão Linear (LR)**:
%- O gráfico de resíduos do LR sugere uma tendência de maior variabilidade à medida que os valores previstos aumentam. Isso reflete a limitação do modelo em lidar com a natureza não linear dos dados, resultando em um número significativo de previsões extremas com grandes erros.
%- A quantidade maior de outliers em valores previstos mais altos pode ser um indicativo de que a LR, por ser um modelo linear, não é capaz de modelar adequadamente as relações complexas entre as variáveis, especialmente em eventos de pico.
%
%- **RandomForest (RF)**:
%- A tendência de dispersão crescente dos resíduos no RF é menos acentuada em comparação com a LR, mas ainda presente. O RF, por ser um modelo baseado em árvores, parece capturar melhor as relações não lineares nos dados, o que resulta em uma menor quantidade de outliers em previsões extremas.
%- No entanto, a presença de alguns outliers, embora menos frequentes, indica que o RF ainda encontra desafios em prever com precisão os picos mais altos de vazão.
%
%### 3. **Comparação Geral**
%
%- **Precisão em Previsões Moderadas**: Todos os três modelos (CB, LR e RF) apresentam resíduos concentrados em torno de zero para valores previstos moderados (entre 3.000 e 6.000 \(m^3/s\)), indicando que eles conseguem lidar bem com a previsão de vazões nessa faixa. No entanto, o RF parece ser o mais consistente, com menor variabilidade entre os resíduos.
%
%- **Erro em Previsões Extremas**: Tanto o CB quanto o RF mostram uma dispersão maior dos resíduos à medida que os valores previstos aumentam, mas o RF lida um pouco melhor com essas previsões extremas. A LR, por outro lado, tem o pior desempenho, com uma grande quantidade de outliers e dispersão considerável em previsões mais altas.
%
%- **Outliers**: O CB apresenta alguns outliers significativos, especialmente em previsões acima de 6.000 \(m^3/s\), mas a LR é a que mais sofre com outliers, indicando que este modelo linear pode não ser adequado para capturar picos de vazão. O RF apresenta menos outliers, sendo, portanto, o modelo que melhor lida com as previsões de vazões extremas.
%
%### **Conclusões Importantes**
%
%1. **CatBoost (CB)**: O CB tem um bom desempenho para previsões de vazões moderadas, mas começa a apresentar maior dispersão dos resíduos à medida que as vazões aumentam. O número de outliers sugere que o modelo pode melhorar no tratamento de eventos extremos de vazão, como enchentes ou picos sazonais.
%
%2. **Regressão Linear (LR)**: A LR é o modelo com pior desempenho em termos de variabilidade dos resíduos e quantidade de outliers, especialmente para valores de previsão extremos. Sua natureza linear o torna inadequado para lidar com as não linearidades presentes nos dados de vazão de rios, particularmente em eventos de pico.
%
%3. **RandomForest (RF)**: O RF é o modelo mais equilibrado, apresentando menor dispersão dos resíduos e menos outliers em comparação aos outros dois modelos. Embora ainda apresente alguma dificuldade em prever vazões extremas, seu desempenho é o mais robusto entre os três modelos.
%
%Em resumo, o **RF** se destaca como o modelo mais eficaz para prever vazões extremas com menor dispersão dos resíduos e menos outliers, enquanto o **CB** apresenta desempenho competitivo, mas com maior variabilidade nos erros. A **LR**, embora seja uma abordagem mais simples, não se adequa bem a este problema, especialmente para previsões de alta vazão.
