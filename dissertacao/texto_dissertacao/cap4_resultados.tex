...
A análise dos gráficos de dispersão dos resíduos ao longo do tempo para os modelos revela informações cruciais sobre a evolução do desempenho de cada modelo em termos de erro durante o período analisado para o rio Grande. Aqui é possível identificar padrões de erro, presença de \textit{outliers} e a estabilidade dos resíduos ao longo do tempo.

O CB demonstra uma concentração dos resíduos dentro da faixa de $\pm 1000 m^3/s$ na maior parte do período estudado - figura \ref{fig:grande_CB_WFV_ORIG_RESID_x_TEMPO}. Contudo, nos primeiros meses, de janeiro a maio de 2023, observa-se uma dispersão maior dos resíduos, com vários pontos excedendo os $2000 m^3/s$. Após esse intervalo inicial, os resíduos do CB tendem à estabilidade, permanecendo majoritariamente dentro dos limites estabelecidos. No entanto, a presença de \textit{outliers} entre março e maio de 2023 indica que o modelo enfrentou dificuldades para lidar com picos de vazão nesse período, ocasionando erros elevados. Apesar disso, após o pico de erro em maio, os resíduos do CB tornaram-se mais constantes, com variações menores e mais previsíveis, embora alguns \textit{outliers} persistam no final do ano, demonstrando, desta forma, que há desafios na captura de variações extremas.

Em contraste, o LR apresenta uma dispersão dos resíduos significativamente maior ao longo do tempo quando comparada ao CB - figura \ref{fig:grande_LR_WFV_ORIG_RESID_x_TEMPO}. Mesmo nos períodos de menor erro, como de julho a novembro de 2023, os resíduos ainda exibem uma variação considerável, estão muito dispersos, com pontos ultrapassando $\pm 1000 m^3/s$. Essa variabilidade reflete a limitação do LR em capturar a complexidade dos dados de vazão. Isso sugere que o LR não lidou adequadamente com eventos extremos de vazão, resultando em erros significativamente maiores em certos momentos. A estabilidade dos resíduos da LR ficou também comprometida, exibindo grande variabilidade mesmo em períodos onde a série observada apresentou comportamento mais estável, indicando que o modelo linear não capturou de forma eficaz as mudanças dinâmicas nas vazões.

Por sua vez, o RF apresenta uma dispersão dos resíduos semelhante à do CB - figura \ref{fig:grande_RF_WFV_ORIG_RESID_x_TEMPO} -, porém com uma quantidade menor de \textit{outliers}, considerando acima de \textit{upper fence} e abaixo de \textit{lower fence}. A maior parte dos resíduos do RF está confinada à faixa de $\pm 1000 m^3/s$, com algumas exceções nos meses de março e abril e também os meses de novembro e dezembro de 2023, onde o modelo aparenta encontrar maior dificuldade em prever corretamente as vazões. A presença reduzida de \textit{outliers} no RF, em comparação ao LR e CB, bem como uma faixa menos extensa entre o menor \textit{outlier} () e o maior \textit{outlier}, sugere uma maior robustez na captura das variações de vazão ao longo do tempo, embora alguns \textit{outliers} ocorram durante períodos de maior instabilidade nas vazões. A estabilidade dos resíduos do RF é notavelmente superior, com menor dispersão e uma tendência clara de erros mais baixos em comparação com a LR, embora ainda sejam observados alguns picos de erro nos primeiros meses de 2023.

De maneira geral, tanto o CB quanto o RF demonstram um padrão de erro mais estável ao longo do tempo, com a maioria dos resíduos concentrados dentro de uma faixa controlada de ±1.000 m³/s. Ambos os modelos apresentam períodos de erro maior, especialmente no início do ano, mas tendem a estabilizar conforme o tempo avança. Em contrapartida, a LR exibe maior variabilidade nos resíduos, indicando uma menor capacidade de lidar com as flutuações nas vazões independentemente do período analisado. A maior quantidade de resíduos dispersos e outliers na LR evidencia dificuldades do modelo linear em capturar a não linearidade intrínseca dos dados de vazão.

Quanto à presença de outliers, a LR possui a maior quantidade, especialmente no início do período, onde os erros são mais frequentes e elevados. Isso reflete a inadequação do modelo linear para prever vazões em cenários de maior complexidade. O CB apresenta outliers em menor quantidade que a LR, enquanto o RF exibe o menor número de outliers, indicando que este último é o modelo mais robusto para lidar com variações extremas, embora ainda enfrente desafios durante os picos de vazão.

No tocante à estabilidade dos resíduos, o RF se destaca como o modelo mais estável ao longo do tempo, com menor variabilidade dos resíduos e menos outliers em comparação aos demais modelos. Essa característica sugere que o RF é mais adequado para lidar com a variação temporal das vazões, mantendo previsões mais confiáveis e consistentes. O CB também demonstra boa estabilidade, embora com mais flutuações nos resíduos, especialmente no início do período, indicando a necessidade de ajustes ou refinamentos em eventos de maior magnitude. Por outro lado, a LR é claramente o modelo menos estável, com alta variabilidade dos resíduos e maior quantidade de erros em períodos críticos, evidenciando sua inadequação para captar a complexidade dos dados de vazão.

Em conclusão, o RandomForest emerge como a melhor escolha em termos de estabilidade e precisão ao longo do tempo, seguido pelo CatBoost, que também apresenta um desempenho aceitável apesar de algumas flutuações. A Regressão Linear, por sua vez, não é recomendada para esse tipo de previsão devido à sua alta variabilidade de erros e grande quantidade de outliers, tornando-a uma opção menos confiável para previsões precisas de vazão.
\begin{figure}[!h]
\centering
...
A análise da distribuição dos resíduos para os três modelos — CatBoost (CB), Regressão Linear (LR) e RandomForest (RF) — no rio Grande revelou padrões relevantes na forma como cada modelo lidou com os erros de previsão. A seguir, apresenta-se uma análise comparativa com base nas distribuições observadas.

O CatBoost apresentou uma assimetria de 1,3, indicando uma distribuição levemente assimétrica para a direita. Isso sugeriu que, embora a maior parte dos resíduos estivesse concentrada próximo a zero, havia uma quantidade considerável de erros positivos, especialmente na faixa de 2.000 a 3.000 m³/s. A curva normal ajustada demonstrou que os resíduos seguiam uma forma próxima da normalidade, porém com uma cauda longa à direita. Esse comportamento indicou que o CB tendia a cometer alguns erros maiores nas previsões de altas vazões. Além disso, foram observados poucos outliers acima de 3.000 m³/s, evidenciando que o modelo enfrentou certa dificuldade em prever corretamente os valores de vazão mais extremos.

Em contraste, a Regressão Linear apresentou uma assimetria de 0,97, o que indicou que a distribuição dos resíduos no modelo LR foi quase simétrica, com um leve viés para a direita. Isso significou que os erros estiveram razoavelmente bem distribuídos em torno de zero, com uma leve tendência de subestimativas. A distribuição dos resíduos acompanhou mais de perto a curva normal ajustada, apresentando uma forma mais equilibrada e uma dispersão menor. No entanto, ainda houve resíduos significativos acima de 1.000 m³/s, sugerindo que a LR pode não ter capturado adequadamente as variações maiores nas vazões. Além disso, houve menos outliers extremos em comparação ao CB, mas o número de resíduos mais distantes da média permaneceu considerável, refletindo a limitação de um modelo linear em capturar a complexidade dos dados de vazão.

O RandomForest, por sua vez, apresentou a menor assimetria entre os três modelos, com um valor de 0,82, indicando uma distribuição de resíduos mais simétrica. Isso sugeriu que o RF lidou melhor com os erros, distribuindo-os de maneira mais uniforme em torno de zero. A distribuição dos resíduos seguiu de forma mais precisa a curva normal ajustada em comparação com o CB e a LR, o que indicou que o modelo RF teve uma maior capacidade de gerar previsões com erros menores e menos dispersos. Além disso, o RF registrou o menor número de outliers em comparação com os demais modelos, o que sugeriu uma maior robustez na gestão de valores extremos de vazão.

De maneira geral, tanto o CB quanto o RF demonstraram padrões de erro mais estáveis ao longo do tempo, com a maioria dos resíduos concentrados dentro de uma faixa controlada de ±1.000 m³/s. Ambos os modelos exibiram períodos de erro maior, especialmente no início do ano, mas tendiam a estabilizar conforme o tempo avançava. Em contrapartida, a LR exibiu maior variabilidade nos resíduos, indicando uma menor capacidade de lidar com as flutuações nas vazões independentemente do período analisado. A maior quantidade de resíduos dispersos e outliers na LR evidenciou as dificuldades do modelo linear em capturar a não linearidade intrínseca dos dados de vazão.

Quanto à presença de outliers, a LR apresentou a maior quantidade, especialmente no início do período, onde os erros foram mais frequentes e elevados. Isso refletiu a inadequação do modelo linear para prever vazões em cenários de maior complexidade. O CB exibiu outliers em menor quantidade do que a LR, enquanto o RF registrou o menor número de outliers, indicando que este último foi o modelo mais robusto para lidar com variações extremas, embora ainda enfrentasse desafios durante os picos de vazão.

No tocante à estabilidade dos resíduos, o RF destacou-se como o modelo mais estável ao longo do tempo, com menor variabilidade dos resíduos e menos outliers em comparação aos demais modelos. Essa característica sugeriu que o RF foi mais adequado para lidar com a variação temporal das vazões, mantendo previsões mais confiáveis e consistentes. O CB também demonstrou boa estabilidade, embora com mais flutuações nos resíduos, especialmente no início do período, indicando a necessidade de ajustes ou refinamentos em eventos de maior magnitude. Por outro lado, a LR foi claramente o modelo menos estável, com alta variabilidade dos resíduos e maior quantidade de erros em períodos críticos, evidenciando sua inadequação para captar a complexidade dos dados de vazão.

Em conclusão, o RandomForest emergiu como a melhor escolha em termos de estabilidade e precisão ao longo do tempo, seguido pelo CatBoost, que ainda apresentou desempenho aceitável apesar de algumas flutuações. A Regressão Linear, por sua vez, mostrou-se a menos eficaz devido à sua limitação em capturar a complexidade dos dados de vazão, tornando-se uma opção menos confiável para previsões precisas.
...
A análise comparativa e a interpretação dos gráficos apresentados para os três modelos de previsão — RandomForest (RF), Regressão Linear (LR) e CatBoost (CB) — com base na função de autocorrelação dos resíduos no rio Grande, revelaram padrões significativos na maneira como cada modelo lidou com as dependências temporais nos dados.

O gráfico de autocorrelação (ACF) do RandomForest demonstrou que os primeiros dez lags apresentaram autocorrelação significativa e positiva. Esse comportamento indicou a presença de uma estrutura nos resíduos que não havia sido completamente capturada pelo modelo, sugerindo que o RandomForest ainda mantinha dependências temporais não modeladas de forma adequada. A partir do lag 15, as correlações caíram dentro da faixa de confiança (área azul), sinalizando uma melhora na independência dos resíduos. No entanto, as flutuações entre autocorrelações positivas e negativas dentro dessa faixa ainda eram perceptíveis, o que sugeria resíduos levemente estruturados ao longo do tempo.

De maneira semelhante, o modelo de Regressão Linear exibiu uma característica comparável em relação aos primeiros lags, com os primeiros dez lags também apresentando autocorrelação positiva significativa. A dependência temporal não havia sido adequadamente modelada, sendo mais evidente nos primeiros lags. À medida que os lags aumentavam, a autocorrelação diminuía, embora ainda houvesse flutuações. Houve uma melhoria considerável após o lag 20, quando os resíduos começaram a oscilar mais próximo de zero e dentro da faixa de confiança. Isso sugeriu que, apesar de sua simplicidade, o modelo linear conseguia capturar alguns padrões, embora ainda houvesse espaço para melhorias na modelagem temporal.

O gráfico ACF do CatBoost também apresentou um padrão semelhante nos primeiros lags, com autocorrelações significativas nos primeiros 10 a 15 lags, indicando que o modelo ainda mantinha algumas dependências temporais não modeladas. No entanto, comparado ao RandomForest e à Regressão Linear, o CatBoost demonstrou uma recuperação mais rápida, com a maior parte dos resíduos alcançando a faixa de confiança em um estágio mais precoce. Esse comportamento sugeriu que o CatBoost possuía uma ligeira vantagem sobre os outros modelos na captura da estrutura dos dados e na modelagem das dependências temporais, embora ainda apresentasse alguma autocorrelação nos primeiros lags.

As conclusões gerais indicaram que todos os modelos apresentaram algum nível de autocorrelação significativa nos primeiros lags, o que sugeria que nenhum deles havia conseguido modelar completamente as dependências temporais presentes nos dados. Isso apontava para a possibilidade de melhorias, como a incorporação de mecanismos para lidar com sazonalidade ou dependência temporal de forma mais direta, por exemplo, através de modelos autorregressivos ou features de lag mais robustas.

Especificamente, o CatBoost destacou-se por reduzir as autocorrelações de forma mais eficaz do que os demais modelos. Esse desempenho estava alinhado com sua arquitetura mais adaptável e sua maior capacidade de modelagem em relação a variáveis complexas, permitindo capturar melhor padrões não lineares e estruturais nos dados. Por outro lado, a Regressão Linear, sendo um modelo simples e linear, mostrou-se a mais afetada pelas dependências temporais e estruturais nos resíduos. Sua incapacidade de capturar padrões não lineares ou interações mais complexas refletiu-se na quantidade de resíduos que exibiam correlação nos primeiros lags.

Embora o RandomForest fosse um modelo baseado em árvores, teoricamente mais flexível que a Regressão Linear, ainda apresentou autocorrelação significativa nos resíduos, especialmente nos primeiros lags. Isso poderia ser um reflexo de sua natureza não paramétrica e, potencialmente, da ausência de features temporais ou lags adequados no modelo.

Em suma, nenhum dos modelos conseguiu eliminar completamente a dependência temporal dos resíduos. No entanto, o CatBoost apresentou uma performance superior na redução dessas dependências, sugerindo que, para aprimorar a precisão dos modelos, seria interessante explorar a incorporação de características temporais adicionais ou técnicas como a modelagem autorregressiva. Dessa forma, seria possível melhorar a captura das dinâmicas temporais presentes nos dados de vazão, resultando em previsões mais precisas e consistentes.
...
O CB apresentou uma KGE de $0,93$, indicando uma boa eficiência nas previsões e demonstrando que o modelo conseguiu replicar o comportamento hidrológico observado de forma consistente. A Regressão Linear destacou-se com o maior valor de KGE, atingindo 0,98472, o que corroborou sua elevada capacidade preditiva e eficiência em relação aos dados observados. O RandomForest apresentou um KGE de 0,97754, reforçando sua qualidade de previsão, que se aproximou consideravelmente dos valores observados, embora ligeiramente inferior ao da Regressão Linear.

Em seguida, a métrica de MAPE (Mean Absolute Percentage Error) foi analisada. O CatBoost exibiu um MAPE de 0,03296, indicando um erro percentual médio absoluto muito baixo. A Regressão Linear apresentou um MAPE de 0,02707, sugerindo que este modelo conseguiu prever com maior precisão os valores centrais da série em comparação ao CatBoost. O RandomForest demonstrou o menor MAPE entre os três modelos, com um valor de 0,02571, refletindo uma excelente capacidade preditiva e menor erro médio absoluto.

Quanto ao PBIAS (Percent Bias), o CatBoost apresentou um valor de -2,61473\%, evidenciando um leve viés negativo, o que indicou que o modelo tendia a subestimar as vazões previstas. A Regressão Linear registrou o menor PBIAS entre os modelos, com -0,98255\%, indicando que suas previsões possuíam o menor viés e eram mais equilibradas em relação aos valores observados. O RandomForest exibiu um PBIAS de -0,80974\%, refletindo também um leve viés de subestimação, porém comparável ao do CatBoost.

Por fim, a cobertura empírica foi avaliada para cada modelo. O CatBoost cobriu 90,68\% dos dados, demonstrando uma boa capacidade de captura das incertezas nas previsões. A Regressão Linear alcançou a maior cobertura empírica, com 96,71\%, sugerindo que o intervalo de previsão gerado pela LR capturou quase todos os eventos da série de vazões. O RandomForest, por sua vez, apresentou a menor cobertura empírica, com 82,47\%, o que indicou que o intervalo de previsão gerado pelo RF foi mais estreito e possivelmente não capturou todas as variações na série de vazão.

Na comparação geral, a Regressão Linear destacou-se como o modelo com maior eficiência na replicação do comportamento hidrológico observado, evidenciado pelo maior valor de KGE e pela cobertura empírica mais alta. Além disso, a LR apresentou o menor PBIAS, indicando previsões menos enviesadas. No entanto, o RandomForest mostrou-se o modelo mais preciso em termos de erro percentual médio absoluto (MAPE), seguido pelo CatBoost. Em termos de cobertura empírica, a LR superou os demais modelos, enquanto o RF apresentou a cobertura mais baixa.

Em conclusão, embora os três modelos tenham demonstrado desempenhos competitivos, cada um possuía características distintas que influenciaram sua robustez, precisão e capacidade de capturar a variabilidade dos dados de vazão ao longo do período analisado. A Regressão Linear destacou-se pela eficiência na replicação do comportamento hidrológico e pela baixa incidência de viés, o que a torna uma opção robusta para previsões consistentes. O RandomForest, com seu menor MAPE, mostrou-se altamente preciso, embora com menor cobertura empírica, indicando que poderia não capturar todas as variações extremas. O CatBoost, por sua vez, apresentou um equilíbrio entre as métricas avaliadas, com boa eficiência preditiva e capacidade de captura das incertezas, mas com uma ligeira tendência a subestimar as vazões em alguns casos.
...
A análise dos gráficos de dispersão dos resíduos em relação aos valores previstos para o rio São Francisco, com base nos modelos CatBoost (CB), Regressão Linear (LR) e RandomForest (RF), revelou padrões distintos no comportamento dos resíduos, fornecendo insights importantes sobre o desempenho de cada modelo.

No modelo CatBoost, a maioria dos resíduos concentrou-se dentro da faixa delimitada pelo intervalo interquartílico (IQR), com poucos pontos ultrapassando os limites superiores e inferiores. Observou-se uma leve tendência de aumento da variabilidade dos resíduos conforme os valores previstos aumentavam, sugerindo que o modelo enfrentava maior dificuldade em capturar corretamente os picos de vazão. No entanto, de maneira geral, os resíduos estavam bem distribuídos em torno da linha de referência zero, indicando um bom ajuste global. A presença de alguns outliers, ainda que em menor quantidade, reforçou a estabilidade do CB, especialmente para vazões mais moderadas.

Em contrapartida, a Regressão Linear apresentou um comportamento semelhante em termos de distribuição dos resíduos, com uma leve concentração em torno de zero para a maioria dos valores previstos. Contudo, identificou-se uma maior quantidade de resíduos dispersos, indicando uma variação mais ampla dos erros. Essa dispersão sugeriu que o modelo linear possuía limitações em capturar a complexidade dos dados, especialmente para valores de vazão mais elevados, onde o modelo tendia a subestimar ou superestimar os valores observados com maior frequência. Embora a LR apresentasse uma cobertura mais ampla dos intervalos de previsão, a quantidade de resíduos fora do intervalo interquartílico (IQR) foi superior à do CatBoost, indicando uma menor robustez.
