...
Em contraste, o LR apresenta uma dispersão dos resíduos significativamente maior ao longo do tempo quando comparada ao CB - figura \ref{fig:grande_LR_WFV_ORIG_RESID_x_TEMPO}. Mesmo nos períodos de menor erro, como de julho a outubro de 2023, os resíduos ainda exibem uma variação considerável, com muitos pontos ultrapassando $\pm 1000 m^3/s$. Essa variabilidade reflete a limitação do LR em capturar a complexidade dos dados de vazão. Além disso, a quantidade de \textit{outliers} no LR é superior à dos outros modelos, com vários pontos acima de $2000 m^3/s$ e abaixo de $-1000 m^3/s$ ao longo de todo o período analisado. Isso sugere que o LR não lidou adequadamente com eventos extremos de vazão, resultando em erros significativamente maiores em certos momentos. A estabilidade dos resíduos da LR ficou também comprometida, exibindo grande variabilidade mesmo em períodos onde a série observada apresentou comportamento mais estável, indicando que o modelo linear não capturou de forma eficaz as mudanças dinâmicas nas vazões.

Por sua vez, o RandomForest apresenta uma dispersão dos resíduos semelhante à do CB, porém com uma quantidade menor de outliers. A maior parte dos resíduos do RF está confinada à faixa de ±1.000 m³/s, com algumas exceções nos meses de março e abril de 2023, onde o modelo aparenta encontrar maior dificuldade em prever corretamente as vazões. A presença reduzida de outliers no RF, em comparação à LR e ao CB, sugere uma maior robustez na captura das variações de vazão ao longo do tempo, embora alguns outliers ocorram durante períodos de maior instabilidade nas vazões. A estabilidade dos resíduos do RF é notavelmente superior, com menor dispersão e uma tendência clara de erros mais baixos em comparação com a LR, embora ainda sejam observados alguns picos de erro nos primeiros meses de 2023.
...
A comparação dos modelos CatBoost (CB), Regressão Linear (LR) e RandomForest (RF) no contexto do rio São Francisco revela importantes insights sobre a performance de cada um em termos de erro, viés e cobertura de previsão. Observa-se que, embora os três modelos tenham mostrado um desempenho competitivo, suas características variam em termos de robustez, precisão e capacidade de capturar a variabilidade dos dados de vazão ao longo de um ano.

**CatBoost (CB)** apresenta uma métrica de **MAPE** de 0,03296, indicando um erro percentual médio absoluto muito baixo. Além disso, o **KGE** (0,93396) sugere uma boa eficiência nas previsões, mostrando que o modelo consegue replicar o comportamento hidrológico observado de forma consistente. No entanto, o **PBIAS** de -2,61473\% mostra um leve viés negativo, indicando que o modelo tende a subestimar as vazões previstas. O intervalo de previsão empírico cobre 90,68\% dos dados, o que demonstra uma boa capacidade de captura das incertezas.

**Regressão Linear (LR)**, por outro lado, apresenta o menor **MAPE** entre os três modelos, com 0,02707, sugerindo que esse modelo consegue prever com maior precisão os valores centrais da série. O **KGE** de 0,98472 corrobora essa capacidade de previsão, destacando a eficiência do modelo linear em relação aos dados observados. O **PBIAS** de -0,98255\% é o menor dentre os modelos, indicando que a LR apresenta o menor viés em suas previsões. Além disso, a **cobertura empírica** de 96,71\% é a mais alta, sugerindo que o intervalo de previsão gerado pela LR captura quase todos os eventos da série de vazões.

**RandomForest (RF)** apresenta um **MAPE** de 0,02571, também indicando um erro médio muito baixo e uma boa capacidade preditiva. O **KGE** de 0,97754 reforça a qualidade de previsão do RF, que se aproxima bastante dos valores observados. O **PBIAS** de -0,80974\% reflete um leve viés de subestimação, mas comparável ao CB. No entanto, a **cobertura empírica** de 82,47\% é a menor entre os três modelos, sugerindo que o intervalo de previsão gerado pelo RF é mais estreito e pode não capturar todas as variações na série de vazão.

**Comparação Geral**:
- **MAPE**: A LR se destaca como o modelo mais preciso, com o menor erro percentual médio, seguida pelo RF e depois pelo CB.
- **KGE**: A LR também apresenta o maior valor de KGE, indicando que é o modelo mais eficiente em replicar o comportamento hidrológico observado, seguido de perto pelo RF e CB.
- **PBIAS**: A LR se destaca mais uma vez, com o menor viés, seguida pelo RF e CB, ambos com uma leve subestimação das vazões.
- **Cobertura Empírica**: A LR captura a maior parte dos eventos de incerteza, seguida pelo CB, enquanto o RF apresenta uma cobertura menor.

**Conclusão**: A Regressão Linear (LR) se mostra como o modelo mais preciso e eficiente no contexto do rio São Francisco, com menor erro, viés e maior cobertura de previsão. No entanto, a LR pode ser limitada ao lidar com dados não lineares, o que pode comprometer sua robustez em cenários mais complexos. O **RF** é uma boa alternativa, com uma alta eficiência, embora sua cobertura de previsão seja menor. O **CB** também se comporta bem, com bom desempenho geral, mas com uma tendência a subestimar os valores de vazão. A escolha do modelo ideal depende do objetivo: se a precisão pontual for mais importante, a LR pode ser preferida, enquanto o RF ou CB podem ser escolhidos para maior robustez em cenários mais dinâmicos e complexos.
